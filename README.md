...
every number on the page is explained in plain words, and there is a glossary of
every term used.
...
- **The middle outcome** — what happened in the typical scenario.
- **The range** — the 1st, 5th, 25th, 75th, 95th and 99th percentile of outcomes,
  drawn as a fan chart, so you see the whole spread and not just one line.
...
- **Real or nominal** — the same run shown in today's purchasing power or in
  euros of the day, toggled without re-simulating.
...
```sh
npm test           # node tests/verify.mjs  →  83 checks
```
...
js/chart.js                the fan chart, hand-rolled SVG
